...
if(isset($_GET['delete']) && !empty($_GET['delete'])){
    ...
    $db->query($sql);
    header('Location: brandtest?Brand Successfully Deleted');
}
...
if(isset($_POST['add_submit'])){
    ...
    if(!empty($errors)){
        ...
        $db->query($sql);
        header('Location: brandtest?Brand Added Successfully');
    }
}?>
<h2 class="text-center">Brands</h2>
<!--Brands Form-->
    <div class="text-center">
        <form class="form-inline" action="brandtest<?=((isset($_GET['edit']))?'?edit='.$edit_id:'');?>" method="POST">
            <div class="form-group">
            <label for="brand"><?=((isset($_GET['edit']))?'Edit ':'Add A ');?>Brand</label>
            <input type="text" name="brand" id="brand" class="form-control" autocomplete="off" value="<?=((isset($_POST['brand'])?$_POST['brand']:''));?>">
...
<?php if(isset($_GET['edit'])):
    echo '<a href="brandtest" class="btn btn-default">Cancel</a>';
                endif;?>
...
while($brand = mysqli_fetch_assoc($results)) :?>
   <tr>
<td class="text-center"><a href="brandtest?edit=<?=$brand['id']; ?>" class="btn btn-sm btn-default"><i class="fa fa-pen"></i></a></td>
<td class="text-center" style="font-size: 16px;"><b><?= $brand['brand']; ?></b></td>
<td class="text-center"><a href="brandtest?delete=<?=$brand['id']; ?>" class="btn btn-sm btn-danger"><i class="fa fa-remove-format"></i></a></td>
</tr><?php
endwhile; ?>
</tbody>
</table>
